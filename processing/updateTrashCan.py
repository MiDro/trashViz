...
auth = ("PIVisionService", "PilotSwag_6") #(os.environ['user'], os.environ['pass'])
...
def format_date(date):
	date_str = "{}/{}/{} {}:{}:{} AM".format(date.month, date.day, date.year, date.hour, date.minute, date.second)
	return date_str
...
def update_trashcan(trashcan):
	...
	data = {
		...
		"FillStatus":	0,#int(trashcan.fillStatus),
		"Info":			trashcan.info,
		"MessageID":	trashcan.messageID,
		"Percent":		float(trashcan.percent),
		"Status":		int(trashcan.status)
	}
